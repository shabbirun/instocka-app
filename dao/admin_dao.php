...
class admin_dao{
    ...
    private $custom_mail_default = array(
        ...
        'body'=>'<p>Hello there!</p>
                 <p>A while ago, you were wondering when {{product_name}} would be available again.</p>
                 <p>It is now! Check it out here: {{product_url}}</p>
                 <p>Thanks!</p>',
        'header_bg'=>'#F5F5F5',
        'footer_bg'=>'#F5F5F5',
    ...
    public function getPendingNotifications($date,$ini=null,$limit=null){
        ...
        $Sql = "SELECT c.first_name, c.last_name, c.email, p.product_title, p.option_sku, IF(p.updated_at > 0, DATE_FORMAT(p.updated_at,'%m/%d/%Y'), DATE_FORMAT(p.created_at,'%m/%d/%Y')) AS created_at, p.updated_at FROM customer c INNER JOIN product p ON c.id = p.customer_id WHERE sent='n' AND p.user_login_id='".$this->user_id."'".$query." ORDER BY IF(p.updated_at > 0,p.updated_at,p.created_at) DESC ".$query2;
        $Sql = mysqli_query($this->con,$Sql);
        //$Sql = $Sql->fetch_object();
        return $Sql;
    }
    public function getNotified($date,$ini=null,$limit=null){
        ...
        $Sql = "SELECT c.first_name, c.last_name, c.email, p.product_title, p.option_sku, if(p.updated_at > 0, DATE_FORMAT(p.updated_at,'%m/%d/%Y'), DATE_FORMAT(p.created_at,'%m/%d/%Y')) AS created_at, p.updated_at FROM customer c INNER JOIN product p ON c.id = p.customer_id WHERE sent='y' AND p.user_login_id='".$this->user_id."'".$query." ORDER BY IF(p.updated_at > 0,p.updated_at,p.created_at) DESC ".$query2;
        $Sql = mysqli_query($this->con,$Sql);
        //$Sql = $Sql->fetch_object();
        return $Sql;
    }
